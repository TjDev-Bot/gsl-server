...
// loop through the item objects and create a table row for each item 
  ...
  <path d="M4.646 4.646a.5.5 0 0 1 .708 0L8 7.293l2.646-2.647a.5.5 0 0 1 .708.708L8.707 8l2.647 2.646a.5.5 0 0 1-.708.708L8 8.707l-2.646 2.647a.5.5 0 0 1-.708-.708L7.293 8 4.646 5.354a.5.5 0 0 1 0-.708"/>
                ...
                $("#modal-poItems h1.modal-title").html(`SC${results.data.po_no} (${results.data.supplier_code})`);
...
$("#modal-poItems").on('click', 'button[name="btn-continue"]', function() {
    ...
    $("#modal-poItems tbody tr").each(function() {
        var item = {
            // get the po_no and supplier_code from the modal title
            total_units: $(this).find('input[name="total_units"]').val(),
            unit_qty: $(this).find('input[name="unit_qty"]').val(),
            unit_uom: $(this).find('input[name="unit_qty"]').data('unit_uom'),
            length: $(this).find('td:eq(9)').text(),
            order_uom: $(this).find('td:eq(5)').data('order_uom'),
            sku: $(this).find('td:eq(7)').text(),
            stock_id: $(this).data('id'),
            description: $(this).find('td:eq(8)').text(),
            length: $(this).find('td:eq(9)').text(),
  ...
  <span class="input-group-text">${item.unit_uom}</span>
                            ...
                            <td class="smalltext" data-length="${item.length}">${item.length}</td>
                            <td>
                            <div class="input-group">
                            <input data-order_uom="${item.order_uom}" type="number" class="form-control" id="order_qty" name="order_qty" value="">
                ...
                $("#modal-process-units").modal('show');
                 $("#modal-process-units h1.modal-title").html(`SC${po_no} (${supplier_code})`);
               // Add the Supplier code and PO Number as data attributes to the modal title
