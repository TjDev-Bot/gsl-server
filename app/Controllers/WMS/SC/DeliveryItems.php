...
class DeliveryItems extends BaseController
{
    public function __construct() {
        ...
        $this->builder = $this->db->table('receiving_items_sc');
    }

    public function index() {
        $po_no =  $this->request->getPost('po_no');
        $builder = $this->db->table('receiving_sc')->select('po_no, supplier_code, supplier_id, status')->getWhere(['po_no' => $po_no]);
        $po_header = $builder->getRowArray(); 
        $query = $this->db->table('po_stock_items_sc')->select('*')->getWhere(['po_no'=> $po_no]);
